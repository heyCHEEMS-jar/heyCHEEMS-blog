...
import type { SidebarLayoutConfig, WidgetComponentConfig, WidgetComponentType } from '../types/config'
...
export class WidgetManager {
	 ...
	 */
	getComponentsByPosition(position: 'top' | 'sticky', sidebar: 'left' | 'right' | 'drawer' = 'left', deviceType: 'mobile' | 'tablet' | 'desktop' = 'desktop'): WidgetComponentConfig[] {
		...
		// 平板端逻辑：在左侧有配置组件的情况下仅保留左侧组件，左侧没有配置组件时则将右侧的组件移到左侧
			...
			if (sidebar === 'left') {
				activeSidebar = this.config.components.left.length > 0 ? 'left' : 'right'
			}
		}
	...
	getAnimationDelay(component: WidgetComponentConfig, index: number): number {
		...
		if (this.config.defaultAnimation.enable) {
			return this.config.defaultAnimation.baseDelay + index * this.config.defaultAnimation.increment
		}
	...
	shouldShowSidebar(deviceType: 'mobile' | 'tablet' | 'desktop'): boolean {
		...
		if (deviceType === 'tablet') {
			return this.config.components.left.length > 0 || this.config.components.right.length > 0
		}
		// desktop
		return this.config.components.left.length > 0 || this.config.components.right.length > 0
	}
	 ...
	 * @param sidebar 侧边栏位置
	 */
	addComponentToLayout(type: WidgetComponentType, sidebar: 'left' | 'right' | 'drawer' = 'left'): void {
...
export function getComponentConfig(componentType: WidgetComponentType): WidgetComponentConfig | undefined {
	return widgetManager.getConfig().properties.find(p => p.type === componentType)
}
...
export function isComponentEnabled(componentType: WidgetComponentType): boolean {
	...
	return config.left.includes(componentType) || config.right.includes(componentType) || config.drawer.includes(componentType)
}
