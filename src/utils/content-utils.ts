...
function tokenizeTitle(title: string): Set<string> {
	...
	const hasSegmenter = typeof Intl !== 'undefined' && 'Segmenter' in Intl && typeof (Intl as any).Segmenter === 'function'
 ...
 */
export async function getRelatedPosts(currentPost: CollectionEntry<'posts'>, maxCount = 5): Promise<PostForList[]> {
	...
	const scored = candidates.map(post => {
		...
		const daysSincePublished = (now - new Date(post.data.published).getTime()) / (1000 * 60 * 60 * 24)
