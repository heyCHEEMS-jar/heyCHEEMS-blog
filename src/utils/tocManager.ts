...
export class TOCManager {
	...
	private getIndicatorElement(): HTMLElement | null {
		return this.getTOCContentElement()?.querySelector('[data-card-toc-indicator]') as HTMLElement | null
	}
	...
	public generateTOCHTML(): string {
		...
		filteredHeadings.forEach(heading => {
			const depth = Number.parseInt(heading.tagName.charAt(1), 10)
			const depthLevel = depth === this.minDepth ? 0 : depth === this.minDepth + 1 ? 1 : 2
			...
			if (!headingText) {
				headingText = heading.id === 'banner-subtitle' ? 'Banner Subtitle' : heading.id || 'Heading'
			}
      ...
      `
		...
		tocHTML += '<div data-card-toc-indicator style="opacity: 0;" class="toc-active-indicator"></div>'
