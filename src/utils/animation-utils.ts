...
export class AnimationManager {
	...
	createAnimation(element: HTMLElement, config: AnimationConfig): void {
		const { duration = 300, delay = 0, easing = 'cubic-bezier(0.4, 0, 0.2, 1)', direction = 'up' } = config
	 ...
	 * 批量动画
	 */
	staggerAnimations(elements: NodeListOf<Element> | HTMLElement[], config: AnimationConfig & { stagger?: number } = {}): void {
		const { stagger = 50, ...animationConfig } = config
