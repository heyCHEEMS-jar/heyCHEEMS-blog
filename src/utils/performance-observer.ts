 ...
 */
export function observeLCP(callback: MetricCallback, reportAllChanges = false): () => void {
	let lcpValue = 0
	...
	const observer = new PerformanceObserver(list => {
		...
		if (lastEntry) {
			...
			lcpValue = (lastEntry as LargestContentfulPaint).renderTime || (lastEntry as LargestContentfulPaint).loadTime
 ...
 */
export function observeResourceTiming(callback: MetricCallback, resourceFilter?: (resource: PerformanceResourceTiming) => boolean): () => void {
	const observer = new PerformanceObserver(list => {
		for (const entry of list.getEntries()) {
