...
export function getSidebarPresence(wm: typeof widgetManager): SidebarPresence {
	const hasLeftSidebarComponents = wm.getComponentsByPosition('top', 'left', 'desktop').length > 0 || wm.getComponentsByPosition('sticky', 'left', 'desktop').length > 0

	const hasRightSidebarComponents = wm.getComponentsByPosition('top', 'right', 'desktop').length > 0 || wm.getComponentsByPosition('sticky', 'right', 'desktop').length > 0

	const hasMobileDrawerComponents = wm.getComponentsByPosition('top', 'drawer', 'mobile').length > 0 || wm.getComponentsByPosition('sticky', 'drawer', 'mobile').length > 0

	const hasTabletLeftSidebarComponents = wm.getComponentsByPosition('top', 'left', 'tablet').length > 0 || wm.getComponentsByPosition('sticky', 'left', 'tablet').length > 0
...
export function calculateGridLayout(config: GridLayoutConfig): GridLayoutResult {
	...
	const { hasLeftSidebarComponents, hasRightSidebarComponents, hasMobileDrawerComponents, hasTabletLeftSidebarComponents } = presence
 ...
 * 获取 Banner 图片
 */
export async function getBannerImages(siteConfig: SiteConfig): Promise<BannerImages> {
 ...
 * 计算主内容区域顶部位置
 */
export function getMainPanelTop(defaultWallpaperMode: string, bannerHeightVh: number): string {
