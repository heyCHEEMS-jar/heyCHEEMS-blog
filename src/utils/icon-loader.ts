...
class IconLoader {
	 ...
	 * 带重试机制的加载
	 */
	private async loadWithRetry(timeout: number, retryCount: number, retryDelay: number): Promise<void> {
	...
	private loadScript(timeout: number): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			script.src = 'https://code.iconify.design/iconify-icon/3-latest/iconify-icon.min.js'
			script.async = true
			script.defer = true
...
// 导出便捷函数
export const loadIconify = (options?: IconifyLoadOptions) => iconLoader.loadIconify(options)
export const preloadIcons = (icons: string[]) => iconLoader.preloadIcons(icons)
export const onIconsReady = (callback: () => void) => iconLoader.onLoad(callback)
