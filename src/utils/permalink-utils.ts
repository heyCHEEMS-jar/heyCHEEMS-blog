 ...
 * @param posts 所有非草稿文章
 */
export function initPostIdMap(posts: CollectionEntry<'posts'>[]): Map<string, number> {
	if (postIdMap) {
		return postIdMap
	}

	// 按发布时间升序排序（最早的在前）
	const sortedPosts = [...posts].sort((a, b) => a.data.published.getTime() - b.data.published.getTime())
...
export function generatePermalinkSlug(post: CollectionEntry<'posts'>): string {
	...
	const slug = format
		...
		.replace(/%monthnum%/g, (published.getMonth() + 1).toString().padStart(2, '0'))
 ...
 * @param post 文章数据
 */
export function hasCustomPermalink(post: CollectionEntry<'posts'> | { data: { permalink?: string } }): boolean {
