...
async function processAlbumFolder(folderPath: string, folderName: string): Promise<AlbumGroup | null> {
	// 检查必要文件
	...
	if (isExternalMode) {
		...
		cover = hasWebpCover ? `/images/albums/${folderName}/cover.webp` : `/images/albums/${folderName}/cover.jpg`
...
function scanPhotos(folderPath: string, albumId: string): Photo[] {
	...
	const imageExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.avif', '.bmp', '.tiff', '.tif']

	const imageFiles = files.filter(file => {
		const ext = path.extname(file).toLowerCase()
		return imageExtensions.includes(ext) && file !== 'cover.jpg' && file !== 'cover.webp'
	})
	...
	imageFiles.forEach((file, index) => {
		...
		const src = fileWebpMap.has(file) ? `/images/albums/${albumId}/${fileWebpMap.get(file)}` : `/images/albums/${albumId}/${file}`
