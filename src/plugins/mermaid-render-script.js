;(() => {
	...
	function getThemePalette() {
		...
		const primary = styles.getPropertyValue('--primary')?.trim() || (isDark ? '#7dd3fc' : '#2563eb')
		const surface = styles.getPropertyValue('--card-bg')?.trim() || styles.getPropertyValue('--surface')?.trim() || (isDark ? '#0b1220' : '#ffffff')
		const text = styles.getPropertyValue('--text-color')?.trim() || styles.getPropertyValue('--foreground')?.trim() || (isDark ? '#e5e7eb' : '#0f172a')
		const muted = styles.getPropertyValue('--muted')?.trim() || styles.getPropertyValue('--muted-foreground')?.trim() || (isDark ? '#1f2937' : '#e5e7eb')
		const backdrop = isDark ? 'rgba(8, 15, 30, 0.9)' : 'rgba(255, 255, 255, 0.94)'
	...
	function openFullscreen(svgElement) {
		...
		overlay.style.setProperty('--mermaid-fs-border', palette.isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.08)')

		const closeButton = document.createElement('button')
		closeButton.className = 'mermaid-fullscreen-close btn-regular rounded-lg h-10 w-10'
	...
	async function renderMermaidDiagrams() {
		...
		try {
			const mermaidElements = Array.from(document.querySelectorAll('.mermaid[data-mermaid-code]'))
					...
					batch.map(async (element, localIndex) => {
						...
						while (attempts < maxAttempts) {
							try {
								...
								// 渲染图表
								const { svg } = await window.mermaid.render(`mermaid-${Date.now()}-${globalIndex}-${attempts}`, code)
								...
								attempts++
								console.warn(`Mermaid rendering attempt ${attempts} failed for element ${globalIndex}:`, error)
