			...
			const textNodeChildren =
				firstParagraphChild.value.split('\n').length > 1
					? [
							{
								type: 'text',
								value: firstParagraphChild.value.split('\n').slice(1).join('\n')
							}
						]
					: []

			const paragraphChildren = [...textNodeChildren, ...firstChild.children.slice(1)]

			const alertParagraphChildren = paragraphChildren.length > 0 ? [{ type: 'paragraph', children: paragraphChildren }] : []
