---
title: 快速上手 Recast 实现 AST 静态源码解析
permalink: 快速上手_Recast_实现_AST_静态源码解析
...
当你使用 Node.js 修改源码文件时，用正则直接操作字符串极其危险且难以维护，如果通过 AST 就可以像操作 JSON 一样精确编辑代码。

- Babel (@babel/parser)：负责执行将代码拆成 Token，将 Token 组装成树，把字符串拆解成一个个节点，如变量名、数值、函数，但是在回写代码时，它会按照自己的规则执行修改与回写，导致丢失原有的缩进、空格和注释，格式全乱。
- Recast：在解析时会记录每个节点的原始位置和格式，在回写阶段会进行自动对比，只更新你修改过的部分。

## Recast 基础用法

- `recast.parse`：把代码字符串解析成一颗 AST 树。
- `recast.visit`：在树上找节点，比如找到名为 `config` 的变量，可以从 `path.node` 拿到需要的数据。
- `recast.types.builders` (这里简称 `b` ) ：假如你想把数字 `1` 改成 `2`，你需要用 builder 造出一个“数字 2”的节点来替换。

#### path.node 常用属性

- `type`：节点的类型（如 `Identifier`, `Literal`），这是判断它是什么的第一步。
...
处理节点时，引用变量是一个比较麻烦的地方。

比如代码中不仅仅只是简单的 `port: 8080`，而是 `port: DEFAULT_PORT` 或 `path: process.env.URL`，直接读取 `node.init.value` 会得到 `undefined`，因为这些值不是字面量（Literal），而是标识符（Identifier）或成员表达式（MemberExpression）。
...
在执行 `recast.parse` 时，Recast 会为 AST 的每个节点打上一个隐藏的标签，记录该节点在原始字符串中的起始位置和结束位置，以及它周围的所有空格、换行、分号。

当你使用 `replace` 修改了某个节点或者它的属性时，Recast 会将该节点标记为脏节点 。

在执行 `recast.print` 时，Recast 的渲染器会遍历整棵树：

- 如果是干净节点，则直接从原始字符串中，根据记录的坐标切出那一段文本。
