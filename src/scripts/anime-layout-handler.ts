...
export function initAnimeLayoutHandler(options: LayoutHandlerOptions) {
	...
	function updateAnimeListLayout(layout: string, shouldAnimate = true) {
		...
		const animeItems = Array.from(document.querySelectorAll('[data-anime-status]')) as HTMLElement[]
	...
	window.addEventListener('layoutChange', (event: CustomEvent<{ layout: string }>) => {
		updateAnimeListLayout(event.detail.layout)
	})
}

export function initLayoutListener(containerId: string, hasRightSidebars: boolean) {
