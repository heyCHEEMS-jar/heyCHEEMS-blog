...
class ThemeOptimizer {
	constructor() {
		...
		this.heavySelectors = ['.float-panel', '#navbar', '.music-player', '#mobile-toc-panel', '#nav-menu-panel', '#search-panel', '.dropdown-content', '.widget', '.post-card', '.custom-md']
	...
	initFromConfig() {
		try {
			...
			if (configCarrier && configCarrier.dataset.hideCodeBlocksDuringTransition !== undefined) {
				this.hideCodeBlocksDuringTransition = configCarrier.dataset.hideCodeBlocksDuringTransition === 'true'
			}
		} catch (error) {
