...
import { BANNER_HEIGHT, BANNER_HEIGHT_HOME, SCROLL_CONFIG, SWUP_SELECTORS } from '../core/swup-config'
...
export class BackToTopHandler {
	...
	constructor(bannerEnabled: boolean) {
		...
		this.scrollHandler = ScrollHandler.throttle(this.handleScroll.bind(this), SCROLL_CONFIG.throttleInterval)
	}
	...
	private cacheElements(): void {
		this.backToTopBtn = document.getElementById(SWUP_SELECTORS.backToTopBtn.slice(1))
		this.toc = document.getElementById(SWUP_SELECTORS.tocWrapper.slice(1))
		this.navbar = document.getElementById(SWUP_SELECTORS.navbarWrapper.slice(1))
	}
	...
	private calculateShowThreshold(scrollTop: number): number {
		const contentWrapper = document.getElementById(SWUP_SELECTORS.contentWrapper.slice(1))
		let threshold = window.innerHeight * (BANNER_HEIGHT / 100) + SCROLL_CONFIG.backToTopOffset
	...
	private updateNavbarVisibility(scrollTop: number): void {
		...
		const isHome = document.body.classList.contains('lg:is-home') && window.innerWidth >= 1280
		const currentBannerHeight = isHome ? BANNER_HEIGHT_HOME : BANNER_HEIGHT

		const threshold = window.innerHeight * (currentBannerHeight / 100) - SCROLL_CONFIG.navbarHideOffset
