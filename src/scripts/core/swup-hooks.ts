...
import { ANIMATION_CONFIG, BANNER_HEIGHT, SWUP_SELECTORS, THEME_CONFIG } from './swup-config'
...
export class SwupHooksManager {
	...
	private handleTOCReinit(): void {
		...
		if (isArticlePage) {
			...
			const hasDesktopTOC = tocElement && typeof (tocElement as any).init === 'function'
	...
	private handleBannerTextVisibility(isHomePage: boolean): void {
		const bannerTextOverlay = this.getCachedElement(SWUP_SELECTORS.bannerTextOverlay)
	...
	private syncThemeState(): void {
		const storedTheme = localStorage.getItem(THEME_CONFIG.themeStorageKey) || THEME_CONFIG.lightMode
		const isDark = storedTheme === THEME_CONFIG.darkMode
		const expectedTheme = isDark ? THEME_CONFIG.darkExpressiveTheme : THEME_CONFIG.lightExpressiveTheme
