export function initFilterHandler() {
	...
	function initFilterButtons() {
		...
		const lazyStore = document.getElementById('anime-lazy-store') as HTMLTemplateElement | null
		...
		filterTags.forEach(tag => {
			const clickHandler = function () {
				...
				const animeItems = Array.from(listContainer!.children).filter(item => item.hasAttribute('data-anime-status'))
				...
				const firstPositions = new Map<HTMLElement, { left: number; top: number }>()
	...
	function setupSwupListeners() {
		if (window.swup) {
			...
			window.swup.hooks.on('page:view', () => setTimeout(initFilterButtons, 150))
		}
	}
