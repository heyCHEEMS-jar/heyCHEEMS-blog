...
import { destroyTransitionEffect, getTransitionEffect } from './effects/transition-effect'
import type { BackToTopHandler } from './handlers/back-to-top-handler'
import { getBackToTopHandler, initBackToTopHandler } from './handlers/back-to-top-handler'
import type { FancyboxHandler } from './handlers/fancybox-handler'
import { cleanupFancybox, getFancyboxHandler, initFancybox } from './handlers/fancybox-handler'
...
export class SwupManager {
	...
	constructor() {
		this.bannerEnabled = !!document.getElementById(SWUP_SELECTORS.bannerWrapper.slice(1))
	...
	showBanner(): void {
		requestAnimationFrame(() => {
			...
			// 处理移动端单图 Banner
			const mobileBanner = document.querySelector('.block.md\\:hidden[alt="Mobile banner image of the blog"]')
