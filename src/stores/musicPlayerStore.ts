...
import { DEFAULT_SONG, LOCAL_PLAYLIST, SKIP_ERROR_DELAY, STORAGE_KEY_VOLUME } from '@/components/widgets/music-player/constants'
...
class MusicPlayerStore {
	...
	private async loadPlaylist(): Promise<void> {
		...
		const meting_api = musicPlayerConfig.meting_api ?? 'https://www.bilibili.uno/api?server=:server&type=:type&id=:id&auth=:auth&r=:r'
		...
		if (mode === 'meting') {
			await this.fetchMetingPlaylist(meting_api, meting_server, meting_type, meting_id)
		} else {
			this.loadLocalPlaylist()
		}
	}

	private async fetchMetingPlaylist(api: string, server: string, type: string, id: string): Promise<void> {
		if (!api || !id) {
			return
		}
		...
		const apiUrl = api.replace(':server', server).replace(':type', type).replace(':id', id).replace(':auth', '').replace(':r', Date.now().toString())
	...
	next(autoPlay = true): void {
		...
		if (this.state.isShuffled) {
			...
			newIndex = this.state.currentIndex < this.state.playlist.length - 1 ? this.state.currentIndex + 1 : 0
		}
	...
	prev(): void {
		...
		const newIndex = this.state.currentIndex > 0 ? this.state.currentIndex - 1 : this.state.playlist.length - 1
		this.state.currentIndex = newIndex
