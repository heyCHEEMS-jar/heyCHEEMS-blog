...
export async function decryptContent(encryptedContent: string, password: string): Promise<DecryptResult> {
...
export async function executeDecryptedScripts(contentDiv: HTMLElement): Promise<void> {
...
function regenerateTOC(): void {
	const tocElement = document.querySelector('table-of-contents') as (HTMLElement & { init?: () => void; regenerateTOC?: () => void }) | null
	if (tocElement) {
