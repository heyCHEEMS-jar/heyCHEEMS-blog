 ...
 */
export function createScrollHandler(callback: () => void, options: AddEventListenerOptions = {}): (event: Event) => void {
 ...
 * 滚动到 TOC 容器内的活动元素
 */
export function scrollActiveIntoView(container: HTMLElement, activeElements: HTMLElement[], tocHeight: number): void {
	...
	const visibleHeight = bottommost.getBoundingClientRect().bottom - topmost.getBoundingClientRect().top
 ...
 * 计算活动指示器的位置
 */
export function calculateActiveIndicatorPosition(container: HTMLElement, minEntry: HTMLElement, maxEntry: HTMLElement): { top: number; height: number } {
