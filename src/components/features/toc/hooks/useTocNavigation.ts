 ...
 * 从 DOM 中提取标题数据
 */
export function extractHeadingsFromDOM(containerSelector = '#post-container'): HeadingData[] {
 ...
 * 滚动到指定标题
 */
export function scrollToHeading(id: string, options: TOCScrollOptions = {}): void {
 ...
 * 创建标题点击处理器
 */
export function createHeadingClickHandler(getConfig?: () => { offset?: number; behavior?: ScrollBehavior }): (event: Event) => void {
	return (event: Event) => {
		const anchor = (event.composedPath() as EventTarget[]).find(el => el instanceof HTMLAnchorElement) as HTMLAnchorElement | undefined
...
export function isPostPage(): boolean {
	const container = document.querySelector('.custom-md, .markdown-content, .prose, #post-container')
