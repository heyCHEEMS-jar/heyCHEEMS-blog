 ...
 * 获取 TOC 徽章文本
 */
export function getBadgeText(index: number, level: number, minLevel: number, useJapaneseBadge: boolean): string {
	if (level !== minLevel) {
		return ''
	}
 ...
 * 生成 TOC 条目数组
 */
export function generateTOCItems(headings: HeadingData[], depth: number, useJapaneseBadge: boolean): TOCItem[] {
	if (headings.length === 0) {
		return []
	}
