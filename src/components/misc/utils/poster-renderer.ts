 ...
 */
export function parseDate(dateStr: string): { day: string; month: string; year: string } | null {
	try {
 ...
 */
export function drawDecorativeCircles(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number, themeColor: string, scale: number): void {
	ctx.save()
	ctx.globalAlpha = 0.1
