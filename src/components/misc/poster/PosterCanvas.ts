import { calculateDimensions, drawDateBadge, drawDecorativeCircles, drawRoundedRect, getLines, loadImage, parseDate, type SizeConfig } from '../utils/poster-renderer'
...
export async function generatePosterImage(options: PosterDrawOptions): Promise<string> {
	const { title, description, pubDate, coverImage, author, siteTitle, avatar, qrCodeUrl, themeColor, scale, width, padding, contentWidth, authorLabel, scanToReadLabel } = options

	const [qrImg, coverImg, avatarImg] = await Promise.all([loadImage(qrCodeUrl), coverImage ? loadImage(coverImage) : Promise.resolve(null), avatar ? loadImage(avatar) : Promise.resolve(null)])
	...
	const { coverHeight, descHeight, canvasHeight } = calculateDimensions(!!coverImage, title, description, ctx, config)
