 ...
 */
export function generateCalendarGrid(year: number, month: number): { day: number; dateKey: string; hasPost: boolean; postCount: number }[] {
 ...
 */
export function isToday(year: number, month: number, day: number, todayYear: number, todayMonth: number, todayDate: number): boolean {
 ...
 */
export function getPostsForDate(dateKey: string, postDateMap: Record<string, { id: string; title: string; date: string }[]>): { id: string; title: string; date: string }[] {
 ...
 */
export function getPostsForMonth(monthKey: string, postsByMonth: Record<string, { id: string; title: string; date: string }[]>): { id: string; title: string; date: string }[] {
 ...
 */
export function processPostsData(posts: { date: string; id: string; title: string }[]): {
	...
	const postDateMap: Record<string, { id: string; title: string; date: string }[]> = {}
	const postsByMonth: Record<string, { id: string; title: string; date: string }[]> = {}
	const stats = {
 ...
 */
export function getCurrentPostId(path: string, allPostsData: { id: string; date: string; title: string }[]): string | null {
	if (!allPostsData || allPostsData.length === 0) {
		return null
	}
	const decodedPath = decodeURIComponent(path)
	const normalizedPath = decodedPath.endsWith('/') ? decodedPath.slice(0, -1) : decodedPath
	const matchedPost = allPostsData.find(post => normalizedPath.endsWith(`/${post.id}`))
