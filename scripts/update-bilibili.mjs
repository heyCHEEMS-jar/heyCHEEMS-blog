...
const OUTPUT_FILE = path.join(path.dirname(fileURLToPath(import.meta.url)), '../src/data/bilibili-data.json')
...
async function getUserIdFromConfig() {
	try {
		...
		const match = configContent.match(/bilibili:\s*\{[\s\S]*?vmid:\s*["']([^"']+)["']/)
...
async function getAnimeModeFromConfig() {
	try {
		const configContent = await fs.readFile(CONFIG_PATH, 'utf-8')
		const match = configContent.match(/anime:\s*\{[\s\S]*?mode:\s*["']([^"']+)["']/)
...
async function getDataPage(vmid, status, typeNum = 1) {
	const response = await withRetry(() => axios.get(`${API_BASE}?type=${typeNum}&follow_status=${status}&vmid=${vmid}&ps=1&pn=1`))
...
async function getData(vmid, status, typeNum, pn, useWebp, coverMirror, SESSDATA) {
	const headers = SESSDATA ? { cookie: `SESSDATA=${SESSDATA};` } : {}

	const response = await withRetry(() => axios.get(`${API_BASE}?type=${typeNum}&follow_status=${status}&vmid=${vmid}&ps=${PAGE_SIZE}&pn=${pn}`, { headers }))
	...
	return (response?.data?.data?.list || []).map(bangumi => {
		...
		const progressPercent = totalEpisodes > 0 && progress > 0 ? Math.round((progress / totalEpisodes) * 100) : 0
		...
		return {
			...
			rating: bangumi?.rating?.score ? parseFloat(bangumi.rating.score.toFixed(1)) : 0,
			cover: cover,
			description: description,
			year: year,
			studio: studio,
...
async function processData(vmid, status, typeNum, useWebp, coverMirror, SESSDATA) {
	...
	for (let i = 1; i < page.data; i++) {
		...
		const data = await getData(vmid, status, typeNum, i, useWebp, coverMirror, SESSDATA)
		list.push(...data)
...
async function main() {
	...
	if (animeMode !== 'bilibili') {
		console.log(`Detected current anime mode is "${animeMode}", skipping Bilibili data update.`)
		return
	}
	...
	const watching = await processData(VMID, 2, 1, useWebp, coverMirror, SESSDATA)
	const completed = await processData(VMID, 3, 1, useWebp, coverMirror, SESSDATA)
