...
async function fetchMetingPlaylistText() {
	try {
		...
		// 构建 API URL
		const apiUrl = meting_api.replace(':server', meting_server).replace(':type', meting_type).replace(':id', meting_id).replace(':auth', '').replace(':r', Date.now().toString())
...
async function fetchBilibiliAnimeText() {
	try {
		...
		// 检查番剧页面是否启用
		const featurePagesMatch = configContent.match(/featurePages:\s*\{([\s\S]*?)\}/)
		...
		// 提取番剧配置
		const animeModeMatch = configContent.match(/anime:\s*\{[\s\S]*?mode:\s*["']([^"']+)["']/)
		const mode = animeModeMatch ? animeModeMatch[1] : 'bangumi'

		if (mode !== 'bilibili') {
			console.log(`ℹ Anime mode is not "bilibili", skipping Bilibili text collection`)
			return new Set()
		}
		...
		if (!fs.existsSync(dataFilePath)) {
			console.log('ℹ Bilibili data file not found, skipping Bilibili text collection')
			return new Set()
		}
		...
		if (processedCount > 0) {
			console.log(`✓ Successfully processed ${processedCount} anime items from Bilibili data`)
		} else {
		...
		console.log(`⚠ Error processing Bilibili data: ${error.message}, skipping Bilibili text collection`)
		return new Set()
	}
}
...
async function fetchBangumiAnimeText() {
	try {
		...
		if (featurePagesMatch) {
			...
			const animeMatch = featureConfig.match(/anime:\s*(true|false)/)
			if (!animeMatch || animeMatch[1] === 'false') {
				console.log('ℹ Anime page disabled, skipping Bangumi API text collection')
				return new Set()
			}
		}

		// 提取番剧配置
		const bangumiUserIdMatch = configContent.match(/bangumi:\s*\{[\s\S]*?userId:\s*["']([^"']+)["']/)
		const animeModeMatch = configContent.match(/anime:\s*\{[\s\S]*?mode:\s*["']([^"']+)["']/)
		...
		if (mode !== 'bangumi' || !userId) {
			console.log(`ℹ Anime mode is not "bangumi" or no userId configured, skipping Bangumi API text collection`)
			return new Set()
		}
		...
		async function fetchCollection(userId, subjectType, type) {
			try {
				...
				while (hasMore) {
					...
					const response = await fetch(`${BANGUMI_API_BASE}/v0/users/${userId}/collections?subject_type=${subjectType}&type=${type}&limit=${limit}&offset=${offset}`, {
						signal: controller.signal,
						headers: {
							'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
						}
				...
				console.log(`⚠ Failed to fetch collection type ${type}: ${error.message}`)
				return []
			}
		}
		...
		for (const type of collectionTypes) {
			...
			console.log(`✓ Fetched ${collections.length} items from collection type ${type}`)
		...
		if (totalItems > 0) {
			console.log(`✓ Successfully processed ${totalItems} anime items from Bangumi API`)
		} else {
		...
		console.log(`⚠ Error processing Bangumi API config: ${error.message}, skipping anime text collection`)
		return new Set()
	}
}
...
async function collectText() {
	...
	const musicConstantsFile = path.join(__dirname, '../src/components/widgets/music-player/constants.ts')
	...
	if (bangumiTextSet.size > 0) {
		console.log(`✓ Added ${bangumiTextSet.size} unique characters from Bangumi anime data`)
	}
	...
	if (bilibiliTextSet.size > 0) {
		console.log(`✓ Added ${bilibiliTextSet.size} unique characters from Bilibili anime data`)
	}
	...
	try {
		...
		for (const fontConfig of fonts) {
			for (const fontFile of fontConfig.files) {
				...
				const distWoff2 = path.join(__dirname, `../dist/assets/font/${woff2File}`)
				const publicWoff2 = path.join(publicFontDir, `${baseName}.woff2`)
				const hasWoff2 = fs.existsSync(distWoff2) || fs.existsSync(publicWoff2)

				if (!hasWoff2) {
					console.log(`⚠ No woff2 found for ${baseName}, keeping ttf reference`)
					continue
				}
				...
				for (const cssFile of cssFiles) {
					...
					const ttfPattern = new RegExp(`url\\(["']?/assets/font/${baseName}\\.ttf["']?\\)\\s*format\\(["']truetype["']\\)`, 'g')

					if (fontConfig.enableCompress) {
						// 子集优化：直接替换为 woff2（子集化后的）
						cssContent = cssContent.replace(ttfPattern, `url("/assets/font/${woff2File}") format("woff2")`)
					} else {
						// 未开启子集优化：使用原始 woff2（如果有），降级到 ttf
						if (fs.existsSync(publicWoff2)) {
							cssContent = cssContent.replace(ttfPattern, `url("/assets/font/${woff2File}") format("woff2"), url("/assets/font/${baseName}.ttf") format("truetype")`)
						}
					}
			...
			if (file.endsWith('.woff2')) {
				...
				for (const cssFile of cssFiles) {
					...
					if (cssContent.match(ttfPattern)) {
						...
						cssContent = cssContent.replace(ttfPattern, `url("/assets/font/${file}") format("woff2"), url("/assets/font/${ttfFile}") format("truetype")`)
						fs.writeFileSync(cssFile, cssContent)
						console.log(`✓ Updated CSS: ${cssFile} (${baseName} - woff2 fallback)`)
					}
				}
			}
		}
