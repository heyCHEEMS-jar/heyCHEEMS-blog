...
const OUTPUT_FILE = path.join(path.dirname(fileURLToPath(import.meta.url)), '../src/data/bangumi-data.json')

async function getUserIdFromConfig() {
	try {
		const configContent = await fs.readFile(CONFIG_PATH, 'utf-8')
		const match = configContent.match(/bangumi:\s*\{[\s\S]*?userId:\s*["']([^"']+)["']/)

		if (match && match[1]) {
			const userId = match[1]
			if (userId === 'your-bangumi-id' || userId === 'your-user-id' || !userId) {
				console.warn('Warning: userId in src/config.ts appears to be a default value.')
				return userId
			}
			return userId
		}
...
async function processData(items, status) {
	...
	for (const item of items) {
		count++
		process.stdout.write(`[${status}] Processing progress: ${count}/${total} (${item.subject_id})\r`)
		...
		const rating = item.rate ? Number.parseFloat(item.rate.toFixed(1)) : item.subject?.score ? Number.parseFloat(item.subject.score.toFixed(1)) : 0

		const progress = item.ep_status || 0
		const totalEpisodes = item.subject?.eps || progress

		const studio = subjectDetail ? getStudioFromInfobox(subjectDetail.infobox) : 'Unknown'

		const description = (subjectDetail?.summary || item.subject?.short_summary || item.subject?.name_cn || '').trimStart()
...
async function main() {
	...
	if (animeMode !== 'bangumi') {
		console.log(`Detected current anime mode is "${animeMode}", skipping Bangumi data update.`)
		return
	}
