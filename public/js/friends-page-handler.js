...
;(() => {
	...
	function initFriendsPage() {
		...
		// 从页面获取复制成功文本
		var copySuccessTextElement = document.getElementById('friends-copy-success-text')
		if (copySuccessTextElement) {
			window.friendsPageState.copySuccessText = copySuccessTextElement.textContent
		}

		// 清理旧的事件监听器
		if (window.friendsPageState.eventListeners.length > 0) {
			console.log('[Friends Global] Cleaning', window.friendsPageState.eventListeners.length, 'old listeners')
		...
		function filterFriends() {
			...
			for (var i = 0; i < friendCards.length; i++) {
				...
				var matchesSearch = !searchTerm || title.indexOf(searchTerm) >= 0 || desc.indexOf(searchTerm) >= 0
				var matchesTag = currentTag === 'all' || tags.split(',').indexOf(currentTag) >= 0
		...
		window.friendsPageState.eventListeners.push([searchInput, 'input', searchHandler])
		...
		for (var i = 0; i < tagFilters.length; i++) {
			;(button => {
				...
				window.friendsPageState.eventListeners.push([button, 'click', clickHandler])
			})(tagFilters[i])
		}
		...
		console.log('[Friends Global] ✅ Initialization complete with', window.friendsPageState.eventListeners.length, 'listeners')
		return true
	}
	...
	function setupMutationObserver() {
		...
		window.friendsPageState.mutationObserver = new MutationObserver(mutations => {
			...
			for (var i = 0; i < mutations.length; i++) {
				...
				if (mutation.addedNodes && mutation.addedNodes.length > 0) {
					for (var j = 0; j < mutation.addedNodes.length; j++) {
						...
						if (node.nodeType === 1) {
							if (node.id === 'friends-grid' || node.id === 'friend-search' || (node.querySelector && node.querySelector('#friends-grid'))) {
								shouldInit = true
								break
							}
						}
